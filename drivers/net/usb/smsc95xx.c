...
static void smsc95xx_init_mac_address(struct usbnet *dev)
{
	...
	/* No valid MAC address, so it is not a USB dongle but it should be the eth
	 * chip of our HSE11 board. If so, assign the specific MAC address and eth
	 * interface name
	 */
	if (dev->udev->descriptor.idProduct == 0xec00) {
		if (dev->udev->bus->busnum == 1) {
			eth_hw_addr_set(dev->net, pcie_mac1addr);
			if (is_valid_ether_addr(dev->net->dev_addr)) {
				strcpy(dev->net->name, "eth1");
				netif_dbg(dev, ifup, dev->net,
					  "MAC address read from u-boot\n");
				return;
			}
		}

		if (dev->udev->bus->busnum == 2) {
			eth_hw_addr_set(dev->net, pcie_mac2addr);
			if (is_valid_ether_addr(dev->net->dev_addr)) {
