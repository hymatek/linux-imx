	...
	struct usb_endpoint_descriptor *endpoint)
{
	...
	if ((hdev->descriptor.idVendor==0x424) &&
			(hdev->descriptor.idProduct==0x3803)) {
		dev_info(&hdev->dev, "Forcing self_powered mode for "
		...
		u16 portchange)
{
	...
	for (i = 0; i < PORT_INIT_TRIES; i++) {
		...
		if (udev->descriptor.bDeviceClass == USB_CLASS_HUB
			...
			/* BSP-2862: Force USB_DEVICE_SELF_POWERED mode for
			 * USB3803 hubs
			 */
			if ((udev->descriptor.idVendor==0x424) &&
					(udev->descriptor.idProduct==0x3803)) {
